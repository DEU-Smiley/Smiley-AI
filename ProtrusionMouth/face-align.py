...
userId = sys.argv[1] #아이디 [ 매개변수로 아이디를 넣어줘야함 ]


original_Image = '/root/Smiley-AI-Images/ProtrusionMouth/original-image/'+ userId +'.jpg'

directory = '/root/Smiley-AI-Images/ProtrusionMouth/trained-image/' + userId + '/'    # 저장 결과를 담을 폴더
createDirectory(directory)  # 폴더가 있는지 확인하고 없다면 폴더 생성
trained_Image = directory + str(current_time.date()) + '.jpg'
#/root/Smiley-AI-Images/ProtrusionMouth/trained-image/
...
#티어나온 점 개수 파일 출력
#xy_file = open('C:/Users/joon/Desktop/ToothFairy_Server/src/main/python/face-align/test-result/xy.txt', 'w')

input = io.imread(original_Image, pilmode="RGB") #RGB로 변환해줘야
...
plt.savefig(trained_Image, edgecolor='blue', format='png', dpi=300, bbox_inches='tight', pad_inches=0)        
